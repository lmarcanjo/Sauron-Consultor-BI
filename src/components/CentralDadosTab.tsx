...
export const CentralDadosTab: React.FC<CentralDadosTabProps> = ({
  ...
  const [workbookOverview, setWorkbookOverview] = useState<WorkbookOverview | null>(null);
  const activeColumns = useMemo(() => getActiveColumns(), [activeDataset]);
  const activePreviewRows = useMemo(() => getPreviewRows(20), [activeDataset]);

  useEffect(() => {
    if (!activeDataset) {
      setWorkbookOverview(null);
      return;
    }

    setWorkbookOverview(buildWorkbookOverview());
  ...
  const visibleFilterConfigs = useMemo(() => {
    ...
    activeColumns.forEach(column => recordKeys.add(column.toLowerCase()));
    if (recordKeys.size === 0 && dataOrigem && dataOrigem.length > 0) {
  ...
  }, [activeColumns, clientFilterConfigs, dataOrigem]);

  const getSheetModuleUsage = (sheetName: string) => {
    if (!workbookOverview) return "Configuração pendente";

    const usage: string[] = [];
    if (workbookOverview.moduleSheets.commercial === sheetName) usage.push("Comercial");
    if (workbookOverview.moduleSheets.people === sheetName) usage.push("Pessoas");
    if (workbookOverview.moduleSheets.financial === sheetName) usage.push("Financeiro");
    if (workbookOverview.moduleSheets.pending === sheetName) usage.push("Pendências");
    if (workbookOverview.moduleSheets.reports.includes(sheetName)) usage.push("Relatórios");

    return usage.length > 0 ? usage.join(", ") : "Configuração pendente";
  };
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 z-10 relative">
          ...
          <div className="bg-slate-800/60 border border-slate-700 p-4 rounded-2xl min-w-[250px] flex flex-col gap-2 shadow-inner">
            <div>
              ...
              <div className="flex items-center justify-between gap-1.5 mt-1.5">
                <div className="flex items-center gap-1.5">
                  <span className={`w-2.5 h-2.5 rounded-full ${activeDataset ? "bg-emerald-500 animate-pulse" : "bg-slate-500"}`}></span>
                  <span className="text-xs font-black text-blue-450 tracking-wide font-mono uppercase">
                    {activeDataset ? "SPREADSHEET_DATA" : "Nenhuma fonte de dados ativa."}
                  </span>
                </div>
                <span className="text-[10px] bg-slate-700/60 text-slate-300 font-bold px-2 py-0.5 rounded">
                  {(activeDataset?.rowCount || 0).toLocaleString()} Linhas
                </span>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between text-[11px] font-medium text-slate-300">
              ...
              <span className="text-slate-200 truncate max-w-[130px] font-bold">{activeDataset?.sourceName || "Nenhuma fonte de dados ativa."}</span>
            </div>
          </div>
        </div>
      </div>
              ...
              <div className="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    {activeDataset ? (
                      <div className="mb-3 p-2 bg-white dark:bg-slate-900 border border-emerald-200 dark:border-emerald-900 rounded-lg">
                        ...
                        <div className="text-[10px] text-slate-700 dark:text-slate-300">
                          <p><span className="font-medium text-slate-500">Origem:</span> {activeDataset.sourceName}</p>
                          <p><span className="font-medium text-slate-500">Linhas:</span> {activeDataset.rowCount}</p>
                          <p><span className="font-medium text-slate-500">Colunas:</span> {activeColumns.length || activeDataset.columnCount}</p>
                        </div>
                      </div>
                    ) : (
                  ...
                  <div className={`p-4 rounded-xl border transition-all ${
                    ...
                    <p className="text-[10px] text-slate-450 leading-relaxed mb-3">
                      Sem backend ativo neste fluxo. Use uma planilha real para alimentar os módulos.
                    </p>
                    <button
              ...
              {activeDataset && (
                <div id="active-spreadsheet-source-card" className="bg-emerald-50/50 dark:bg-emerald-950/10 border border-emerald-200 dark:border-emerald-900 rounded-2xl p-6 space-y-4 shadow-sm font-sans">
                  ...
                  {workbookOverview && (
                    <div className="border-t border-emerald-100 dark:border-emerald-900/30 pt-4">
                      <div className="flex items-center justify-between gap-3 mb-3">
                        <div>
                          <h5 className="text-xs font-black uppercase text-slate-800 dark:text-slate-100">Abas do workbook e uso por módulo</h5>
                          <p className="text-[11px] text-slate-500 dark:text-slate-400 font-semibold">
                            {workbookOverview.totalSheets} abas, {workbookOverview.totalRows.toLocaleString("pt-BR")} linhas reais.
                          </p>
                        </div>
                      </div>
                      <div className="overflow-auto border border-slate-200 dark:border-slate-800 rounded-xl bg-white dark:bg-slate-900">
                        <table className="w-full text-left text-[11px]">
                          <thead className="bg-slate-50 dark:bg-slate-950 sticky top-0">
                            <tr>
                              <th className="px-3 py-2 font-black text-slate-500 uppercase">Aba</th>
                              <th className="px-3 py-2 font-black text-slate-500 uppercase">Linhas</th>
                              <th className="px-3 py-2 font-black text-slate-500 uppercase">Colunas</th>
                              <th className="px-3 py-2 font-black text-slate-500 uppercase">Módulos usando</th>
                            </tr>
                          </thead>
                          <tbody>
                            {workbookOverview.sheets.slice(0, 18).map(sheet => (
                              <tr key={sheet.sheetName} className="border-t border-slate-100 dark:border-slate-800">
                                <td className="px-3 py-2 font-bold text-slate-800 dark:text-slate-200">{sheet.sheetName}</td>
                                <td className="px-3 py-2 font-mono text-slate-600 dark:text-slate-400">{sheet.rowCount.toLocaleString("pt-BR")}</td>
                                <td className="px-3 py-2 font-mono text-slate-600 dark:text-slate-400">{sheet.columnCount.toLocaleString("pt-BR")}</td>
                                <td className="px-3 py-2 text-slate-600 dark:text-slate-400">{getSheetModuleUsage(sheet.sheetName)}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
                  ...
                  <div className="flex flex-wrap gap-2 pt-2 border-t border-emerald-100 dark:border-emerald-900/30">
                    ...
                    <button
                      onClick={() => {
                        const msg = "Deseja remover esta fonte de dados? O sistema ficará sem fonte ativa até conectar dados reais.";
                        if (confirm(msg)) {
                          ...
                          <thead className="bg-slate-50 dark:bg-slate-950/40 sticky top-0 border-b border-slate-200 dark:border-slate-800 font-bold text-slate-500">
                            <tr>
                              ...
                              {activeColumns.map((column, idx) => (
                                <th key={idx} className="p-2 min-w-[100px]">{column}</th>
                              ))}
                            </tr>
                          </thead>
                          <tbody>
                            {activePreviewRows.map((row, rIdx) => (
