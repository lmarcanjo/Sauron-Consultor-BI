  ...
  const checkStatus = (id: string): { status: "passed" | "warning" | "failed"; error?: string } => {
    ...
    switch (id) {
      ...
      case "dre_real":
        ...
        return { status: "warning", error: "DRE rodando em modo DEMO_DATA. Ative uma planilha real para homologar." };

      case "people_intel":
        if (isRealSpreadsheet) return { status: "passed" };
        return { status: "warning", error: "People Intelligence rodando em modo DEMO_DATA. Ative uma planilha real para homologar." };
