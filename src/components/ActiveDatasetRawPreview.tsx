...
export const ActiveDatasetRawPreview: React.FC = () => {
  ...
  useEffect(() => {
    const unsub = activeDatasetStore.subscribe((event) => {
      if (['DATASET_ACTIVATED', 'DATASET_REHYDRATED', 'DATASET_UPDATED', 'DATASET_REMOVED'].includes(event.type)) {
  ...
  useEffect(() => {
    ...
    IndexedSpreadsheetStorage.getRowsPaged(dataset.rawStorageRef, sheetName, 0, 20)
      .then(rows => {
        if (rows.length > 0) {
          setSheetRows(rows);
          return;
