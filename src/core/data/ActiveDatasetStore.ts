...
export class ActiveDatasetStore {
  ...
  public async rehydrateFromStorage(): Promise<ActiveDataset | null> {
    ...
    try {
      ...
      // Rehydrate full raw data from IndexedDB
      if (typeof window !== "undefined") {
        const fullRows = await IndexedSpreadsheetStorage.getRows(dataset.rawStorageRef);
        if (fullRows && fullRows.length > 0) {
          this.activeRows = fullRows;
          console.log(`[ActiveDatasetStore] Rehydrated ${fullRows.length} full rows from IndexedDB.`);
        } else {
          this.activeRows = dataset.previewRows.map(r => r.raw);
        }
      } else {
