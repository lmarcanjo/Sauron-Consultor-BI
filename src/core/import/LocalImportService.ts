...
export class LocalImportService implements ImportService {
    ...
    try {
      ...
      const sheets: UploadedSheetMetadata[] = workbook.SheetNames.map((sheetName) => {
        ...
        return {
          ...
          rowCount,
          columnCount,
          columns: parseWorksheetRows(worksheet, XLSX.utils, sheetName).headers,
          formulaCount,
  ...
  async activateImport(jobId: string, selectedSheets: string[]): Promise<ActiveDataset> {
    ...
    const activeWorkbook: ActiveWorkbookDataset = withSourceIdentity({
      ...
      columnProfiles: ((primarySheet as SheetMetadata & { columns?: string[] }).columns || getColumns(previewDatasetRows.map(row => row.raw))).map(column => ({
        name: column,
        originalName: column,
        type: previewDatasetRows.some(row => typeof row.raw[column] === "number") ? "number" : "text",
      })),
      importProfile: null,
      rawStorageRef: jobId,
