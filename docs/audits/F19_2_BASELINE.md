...
- No `ColumnRoleSuggestionEngine.ts`: `revenue: "Receita candidata"`, `expense: "Despesa candidata"`.
- No `BusinessMetricBuilder.ts`: `despesaCandidata`, `margemCandidata`, `receitaCandidata`.
...
- O `ConsultingReadinessService.ts` penaliza a prontidão de fontes sem mapeamento de KPI ou DRE.
...
- O `EnterpriseConsolidationService.ts` foi alterado para sempre filtrar as linhas brutas baseando-se no nome ou identificador da empresa selecionada, evitando a exibição de linhas de outras empresas do mesmo grupo que estejam na mesma planilha.
